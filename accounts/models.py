...
class WajoUser(models.Model):
    ...
    created_via = models.CharField(
        max_length=50,
        choices=[("MANUAL", "Manual Registration"), ("EXCEL", "Excel Data Import")],
        default="MANUAL",
    )
    ...
    def clean(self):
        # Validate role exclusivity: Player, Coach, Referee
        if self.role not in ["Player", "Coach", "Referee", None]:
            # Optional: handle other roles if they exist
            pass
        ...
        if self.role == "Referee" and self.coach.exists():
            raise ValidationError("A referee cannot have a coach assigned.")

        # Role change cleanup
        if self.pk and WajoUser.objects.filter(pk=self.pk).exists():
            previous = WajoUser.objects.get(pk=self.pk)
            if previous.role == "Coach" and self.role != "Coach":
                self.players.clear()
            if previous.role == "Player" and self.role != "Player":
                self.coach.clear()
                self.team = None
    ...
    def __str__(self):
        identifier = self.name or self.email or self.phone_no or f"User-{self.id}"
        return f"{identifier} ({self.role or 'No Role'})"
...
class PlayerIDMapping(WajoModel):
    user = models.ForeignKey(
        WajoUser, on_delete=models.CASCADE, related_name="player_ids"
    )
    player_id = models.CharField(max_length=10, unique=True)
    player_position = models.CharField(
        max_length=10, choices=POSITION_CHOICES, default="ST"
    )
...
class OTPStore(models.Model):
    ...
    def is_valid(self):
        time_valid = timezone.now() - self.created_on < timezone.timedelta(
...
class UserRequest(models.Model):
    ...
    REQUEST_TYPE_CHOICES = (("account_deletion", "Account Deletion"),)
    # Link the request to the user
    user = models.ForeignKey(
        WajoUser, on_delete=models.CASCADE, related_name="requests"
    )
