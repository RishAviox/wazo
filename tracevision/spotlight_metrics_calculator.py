...
def clear_spotlight_data_cache(player_id: str, session_id: str) -> bool:
    ...
    try:
        ...
        logger.exception(f"Error clearing spotlight cache for player {player_id}: {e}")
        return False
...
class SpotlightMetricsCalculator:
    ...
    def __init__(self, field_length_m: float = 105.0, field_width_m: float = 68.0):
        ...
        self.SCALE_FACTOR = (
            self.FIELD_LENGTH_M + self.FIELD_WIDTH_M
        ) / 2000.0  # meters per unit
    ...
    def load_spotlight_data(self, file_path: str) -> List[List[float]]:
        ...
        try:
            ...
            logger.exception(f"Error loading spotlight data from {file_path}: {e}")
            return []

    def load_spotlight_data_from_azure_blob(
        self, blob_url: str, cache_key: str = None
    ) -> List[List[float]]:
        """
    ...
    def calculate_gps_athletic_skills(
        self, spotlights: List[List[float]]
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            acceleration_data = self._calculate_acceleration_metrics(spotlights)

            # Calculate play time
            total_time_seconds = (spotlights[-1][0] - spotlights[0][0]) / 1000.0
    ...
    def calculate_gps_football_abilities(
        self,
        spotlights: List[List[float]],
        ...
        try:
            ...
            kick_metrics = self._estimate_kick_metrics(spotlights, highlights, objects)
    ...
    def _analyze_movement(self, spotlights: List[List[float]]) -> Dict[str, float]:
        ...
        try:
            ...
            for i in range(1, len(spotlights)):
                ...
                distance = self._calculate_distance_meters(prev_point, curr_point)
    ...
    def _calculate_distance_meters(
        ...
        try:
            ...
            dx = (point2[1] - point1[1]) * self.SCALE_FACTOR  # Convert to meters
            dy = (point2[2] - point1[2]) * self.SCALE_FACTOR  # Convert to meters
        ...
        try:
            ...
            for i in range(1, len(speeds)):
                ...
                if time_diff > 0:
                    ...
                    distance_segment = distances[i] if i < len(distances) else 0
            ...
            avg_intensity = (
                sum(abs(a) for a in accelerations) / len(accelerations)
                if accelerations
                else 0
            )
            ...
            return {
                ...
                "max_accel_count": 0,
                "max_decel_count": 0,
                "high_accel_distance_km": 0,
                "high_decel_distance_km": 0,
                "max_accel_distance_m": 0,
                "max_decel_distance_m": 0,
            }

    def _analyze_football_actions(
        self, spotlights: List[List[float]]
    ) -> Dict[str, float]:
            ...
            for i in range(1, len(spotlights)):
                ...
                if time_diff > 0:
                    ...
                    if speed > self.HIGH_INTENSITY_THRESHOLD:
                        ...
                        if (
                            current_high_intensity_duration > 3.0
                        ):  # 3+ seconds of high intensity
    ...
    def _estimate_dribbling_metrics(
        self, spotlights: List[List[float]]
    ) -> Dict[str, float]:
    ...
    def _is_dribbling_sequence(self, window: List[List[float]]) -> bool:
        ...
        try:
            ...
            for i in range(1, len(window)):
                distance = self._calculate_distance_meters(window[i - 1], window[i])
            ...
            return (
                1.0 < avg_speed < self.DRIBBLE_MAX_SPEED
    ...
    def _get_speed_at_index(self, spotlights: List[List[float]], index: int) -> float:
        ...
        try:
            ...
            time_diff = (spotlights[index][0] - spotlights[index - 1][0]) / 1000.0
    ...
    def _calculate_athletic_skills_score(
        self, movement_data: Dict, distance_zones: Dict, accel_data: Dict
    ) -> float:
        """Calculate overall athletic skills score (0-10)"""
        try:
            # Weighted scoring
            distance_score = min(movement_data["total_distance_km"] / 12.0 * 3, 3)
            speed_score = min(movement_data["max_speed_kmh"] / 35.0 * 3, 3)
            intensity_score = min(distance_zones["high_intensity_km"] / 2.0 * 2, 2)
    ...
    def _calculate_football_skills_score(
        self, movement_data: Dict, kick_metrics: Dict, dribbling_metrics: Dict
    ) -> float:
        """Calculate overall football skills score (0-10)"""
        try:
            movement_score = min(movement_data["avg_speed_mps"] / 5.0 * 3, 3)
            kick_score = min(
                (kick_metrics["power_kicks"] + kick_metrics["high_intensity_kicks"])
                / 5.0
                * 3,
                3,
            )
            dribble_score = min(dribbling_metrics["dribble_count"] / 10.0 * 2, 2)
            distance_score = min(movement_data["total_distance_km"] / 10.0 * 2, 2)
    ...
    def _get_empty_athletic_metrics(self) -> Dict[str, str]:
        ...
        return {
            ...
            "Session Volume": "0.0 km",
            "Top Speed": "0 km/h",
            "Int. Speed": "0 Km/h",
            "Walking": "0.0 km",
            "Jogging": "0.0 km",
            "High Int. Run": "0|0.00 km",
            "Max Int. Run": "0.0 km",
            "Session Intensity": "0 m",
            "High Int. Acceleration": "0|0.0 km",
            "High Int. Deceleration": "0|0.0 km",
            "Max Int. Acceleration": "0|0 m",
            "Max Int. Deceleration": "0|0 m",
            "Session Int. Acceleration": "0 ",
            "Athletic Skills": "0.0",
        }
