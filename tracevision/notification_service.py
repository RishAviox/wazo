...
class NotificationService:
    ...
    def create_silent_notification(self, user, device, session_data):
        ...
        try:
            ...
            if len(compact_postback) > 24:
                ...
                truncated_session_id = str(session_data.get("session_id"))[
            ...
            body_text = (
                f"Session {session_data.get('session_id')} {status_code.upper()}"
            )
            ...
            logger.info(
                f"Silent notification created for device {device.fcm_token} for session {session_data.get('session_id')}"
            )
            return notification

        except Exception as e:
            logger.exception(
                f"Error creating silent notification for device {device.fcm_token}: {e}"
            )
            return None
    ...
    def create_notification(self, user, device, title, body, postback=""):
        ...
        try:
            notification = Notification.objects.create(
                user=user, device=device, title=title, body=body, postback=postback
            )
            ...
            logger.exception(
                f"Error creating notification for device {device.fcm_token}: {e}"
            )
            return None
    ...
    def create_silent_notification_for_all_devices(self, user, session_data):
        ...
        try:
            ...
            for device in user_devices:
                notification = self.create_silent_notification(
                    user, device, session_data
                )
                if notification:
                    notifications.append(notification)

            logger.info(
                f"Created silent notifications for {len(notifications)} devices of user {user.phone_no}"
            )
            return notifications

        except Exception as e:
            logger.exception(
                f"Error creating silent notifications for all devices of user {user.phone_no}: {e}"
            )
            return []
        ...
        try:
            ...
            for device in user_devices:
                notification = self.create_notification(
                    user, device, title, body, postback
                )
                if notification:
                    notifications.append(notification)

            logger.info(
                f"Created notifications for {len(notifications)} devices of user {user.phone_no}"
            )
            return notifications

        except Exception as e:
            logger.exception(
                f"Error creating notifications for all devices of user {user.phone_no}: {e}"
            )
            return []
    ...
    def mark_notification_as_read(self, notification_id, user):
        ...
        try:
            ...
            logger.info(
                f"Marked notification {notification_id} as read for user {user.phone_no}"
            )
            return True

        except Notification.DoesNotExist:
            logger.warning(
                f"Notification {notification_id} not found for user {user.phone_no}"
            )
            return False
        except Exception as e:
            logger.exception(
    ...
    def get_unread_notifications(self, user, device, limit=10):
        ...
        """
        try:
            notifications = device.notifications.filter(is_read=False).order_by(
                "-created_on"
            )[:limit]
            return notifications
            ...
            logger.exception(
                f"Error getting unread notifications for device {device.fcm_token}: {e}"
            )
            return []
