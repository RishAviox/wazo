...
class TraceVisionService:
    ...
    def __init__(self):
        ...
        # Cache timeouts
        self.status_cache_timeout = getattr(settings, 'TRACEVISION_STATUS_CACHE_TIMEOUT', 300)
        self.result_cache_timeout = getattr(settings, 'TRACEVISION_RESULT_CACHE_TIMEOUT', 1800)
    ...
    def get_session_status(self, session, force_refresh=False):
        ...
        if force_refresh:
            ...
            if cached_data:
                logger.info(f"Using cached status data for session {session.session_id}")
                return cached_data
        
        # Fetch from API
        logger.info(f"Fetching status from TraceVision API for session {session.session_id}")
    ...
    def get_session_result(self, session):
        ...
        # Fetch from API
        logger.info(f"Fetching result from TraceVision API for session {session.session_id}")
    ...
    def _get_cached_status_data(self, session_id):
        ...
        if cached_data:
            ...
            cache_timestamp = datetime.fromisoformat(cached_data.get('cache_timestamp', '1970-01-01T00:00:00'))
    ...
    def _cache_status_data(self, session_id, data):
        ...
        logger.info(f"Cached status data for session {session_id} with TTL {self.status_cache_timeout}s")
    ...
    def _cache_result_data(self, session_id, data):
        ...
        logger.info(f"Cached result data for session {session_id} with TTL {self.result_cache_timeout}s")
    ...
    def _query_tracevision_status(self, session):
        ...
        try:
            ...
            if not data.get("status"):
                logger.error(f"No status data returned for session {session.session_id}")
                return None
                
            return data
            
        except Exception as e:
            logger.exception(f"Error querying TraceVision status for session {session.session_id}: {e}")
            return None
    ...
    def _fetch_session_result(self, session):
        ...
        try:
            ...
            result_response = requests.post(self.graphql_url, headers={"Content-Type": "application/json"}, json=result_payload)
            result_data = result_response.json().get("data", {}).get("sessionResult")

            if result_response.status_code == 200 and result_data:
                logger.info(f"Successfully fetched result data for session {session.session_id}")
                return result_data
            else:
            ...
            logger.exception(f"Error fetching result data for session {session.session_id}: {e}")
            return None
