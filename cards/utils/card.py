...
def get_prompt_for_insight(user, card):
    ...
    if card == "StatusCard":
        ...
        if user.role == 'Coach':
            ...
            prompt = f"Generate a concise expert analysis for coaches only in {language} language based on the provided performance metrics for the team's defensive skills, highlighting key strengths, areas for improvement, and suggesting targeted drills. Keep it under 40 words and avoid mentioning the data passed to you or athletes and coaches. Make sure the sentence can be directly sent to the coach. Data provided:{user_data}. Example: 'The team demonstrates strong tackling and interception numbers but needs better defensive organization and discipline. Focus on improving defensive transitions and positioning. Suggested drills: Defensive Shape, 1v1 Defending, and Transition Defense. While you get the overall data, always focus on defensive skill for the team. {'Dont translate but think and respond in Hebrew.' if language == 'Hebrew' else ''}"
            return prompt
            ...
            prompt = f"Generate a concise expert analysis for athletes only in {language} language based on the provided performance metrics, focusing on their defensive skills, highlighting key strengths, areas for improvement, and suggesting targeted drills. Keep it under 40 words and avoid mentioning the data passed to you or athletes and coaches. Make sure the sentence can be directly sent to the player. Data provided:{user_data}. Example: 'Your tackling and interception skills are strong, but work on improving positioning and defensive discipline. Focus on drills for 1v1 defending, marking, and maintaining shape during transitions.While you get the overall data, always focus on defensive skill for the user. {'Dont translate but think and respond in Hebrew.' if language == 'Hebrew' else ''}"
            return prompt
